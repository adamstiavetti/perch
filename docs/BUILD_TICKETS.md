...
- [Operator Private-App Scope Gate Fix](ops/operator-private-app-scope-gate-fix.md) - narrows the private-app operator override to `operator.internal_private_app_access` only, keeping unrelated operator/admin tooling scopes separate from app-entry override. Runtime validation and deployment remain pending after review/merge.
