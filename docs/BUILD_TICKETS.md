...
- [Security Headers Hardening](ops/security-headers-hardening.md) - pending-runtime browser header hardening that adds `nosniff`, strict-origin referrer policy, restrictive permissions policy, enforced anti-framing, and report-only broad CSP while leaving Vercel-provided HSTS unchanged.
