...
- Continue the FBMVP sequence by reviewing, merging, applying, and runtime-proving `FBMVP-T03A Beta Invite-Code Foundation`; after that, proceed to `FBMVP-T04 Onboarding/Signup Flow Update`. Content/moderation policy can still be refined before live community launch if needed.
...
Recommended next task after `FBMVP-T03A` review, merge, migration apply, and runtime proof: `FBMVP-T04 Onboarding/Signup Flow Update`.
