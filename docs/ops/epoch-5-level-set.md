...
| Security headers hardening | patch pending runtime deploy | `docs/ops/security-headers-hardening.md`, `next.config.ts`, `test/security-headers/securityHeaders.test.mts` | Adds app-owned `nosniff`, strict-origin referrer policy, restrictive permissions policy, enforced anti-framing, and report-only broad CSP. Vercel-provided HSTS is left unchanged. Runtime validation on apex, `www`, and beta remains pending after review/merge/deploy. |
