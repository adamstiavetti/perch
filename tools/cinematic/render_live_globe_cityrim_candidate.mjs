...
const baselineDesktopPath = path.join(previewsDir, "live-globe-proof-baseline-desktop.png");
const baselineMobilePath = path.join(previewsDir, "live-globe-proof-baseline-mobile.png");
const candidateDesktopPath = path.join(previewsDir, "live-globe-proof-cityrim-desktop.png");
const candidateMobilePath = path.join(previewsDir, "live-globe-proof-cityrim-mobile.png");
const comparisonPath = path.join(previewsDir, "live-globe-proof-cityrim-comparison.png");
...
async function captureCandidateScreens(browser) {
  const desktopPage = await browser.newPage({ viewport: desktopViewport, deviceScaleFactor: 1 });
...
async function buildComparisonSheet() {
  const comparisonHtmlPath = path.join(previewsDir, "_tmp-live-globe-cityrim-comparison.html");
  const html = `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Live Globe Cityrim Comparison</title>
    <style>
      :root {
        color-scheme: dark;
      }
      * {
      ...
      .card {
        ...
        border: 1px solid rgba(115, 174, 230, 0.24);
      }
      .card img {
        display: block;
        width: 100%;
  ...
  <body>
    <div class="grid">
      <section class="card">
        ...
        <div class="meta">
          <p class="title">Baseline Desktop</p>
          <p class="subtitle">current default before candidate</p>
        </div>
      </section>
      <section class="card">
        <img src="file://${candidateDesktopPath}" alt="Cityrim desktop" />
        <div class="meta">
          <p class="title">Cityrim Desktop</p>
          <p class="subtitle">accepted default state</p>
        </div>
      </section>
      <section class="card">
        <img src="file://${baselineMobilePath}" alt="Baseline mobile" />
        <div class="meta">
          <p class="title">Baseline Mobile</p>
          <p class="subtitle">current default before candidate</p>
        </div>
      </section>
      <section class="card">
        <img src="file://${candidateMobilePath}" alt="Cityrim mobile" />
        <div class="meta">
          <p class="title">Cityrim Mobile</p>
          <p class="subtitle">accepted default state</p>
        </div>
      </section>
    </div>
