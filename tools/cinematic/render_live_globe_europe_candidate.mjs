...
const baselineDesktopPath = path.join(previewsDir, "live-globe-proof-europe-baseline-desktop.png");
const baselineMobilePath = path.join(previewsDir, "live-globe-proof-europe-baseline-mobile.png");
const candidateDesktopPath = path.join(previewsDir, "live-globe-proof-europe-desktop.png");
const candidateMobilePath = path.join(previewsDir, "live-globe-proof-europe-mobile.png");
const comparisonPath = path.join(previewsDir, "live-globe-proof-europe-comparison.png");
const comparisonHtmlPath = path.join(previewsDir, "_tmp-live-globe-europe-comparison.html");
...
async function captureCandidateScreens(browser) {
  const desktopPage = await browser.newPage({ viewport: { width: 1440, height: 900 }, deviceScaleFactor: 1 });
  await desktopPage.emulateMedia({ reducedMotion: "reduce" });
  await desktopPage.goto(`${baseUrl}/lab/live-globe-proof?candidate=europe&grade=europe`, { waitUntil: "networkidle" });
  await desktopPage.waitForTimeout(2200);
  ...
  const mobilePage = await mobileContext.newPage();
  await mobilePage.emulateMedia({ reducedMotion: "reduce" });
  await mobilePage.goto(`${baseUrl}/lab/live-globe-proof?candidate=europe&grade=europe`, { waitUntil: "networkidle" });
  await mobilePage.waitForTimeout(2200);
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Live Globe Europe Comparison</title>
    <style>
  ...
  <body>
    <div class="grid">
      <section class="card">
        ...
        <div class="meta">
          <p class="title">Europe Baseline Desktop</p>
          <p class="subtitle">accepted Atlantic default before Europe candidate</p>
        </div>
      </section>
      <section class="card">
        <img src="file://${candidateDesktopPath}" alt="Europe candidate desktop" />
        <div class="meta">
          <p class="title">Europe Candidate Desktop</p>
          <p class="subtitle">candidate=europe grade=europe</p>
        </div>
      </section>
      <section class="card">
        <img src="file://${baselineMobilePath}" alt="Baseline mobile" />
        <div class="meta">
          <p class="title">Europe Baseline Mobile</p>
          <p class="subtitle">accepted Atlantic default before Europe candidate</p>
        </div>
      </section>
      <section class="card">
        <img src="file://${candidateMobilePath}" alt="Europe candidate mobile" />
        <div class="meta">
          <p class="title">Europe Candidate Mobile</p>
          <p class="subtitle">candidate=europe grade=europe</p>
        </div>
      </section>
    </div>
